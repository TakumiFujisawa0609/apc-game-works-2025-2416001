...
{
	// DxLib側の衝突情報セットアップ
	MV1SetupCollInfo(trans_.modelId);

	// モデルのコライダ
	ColliderModel* colModel =
		new ColliderModel(ColliderBase::TAG::STAGE, &trans_);
...
void Stage::Collision(void)
{
	for (auto& frameIdx : frameOpacityRate_) {
		MV1SetFrameOpacityRate(trans_.modelId, frameIdx, 1.0f);
	}
	...
	for (const auto& hitCol : hitColliders_)
	{
		...
		for (int i = 0; i < hits.HitNum; i++)
		{
			...
			MV1SetFrameOpacityRate(trans_.modelId, hit.FrameIndex, 0.3f);
...
{
	// フレーム数を取得
	int num = MV1GetFrameNum(trans_.modelId);
	for (int i = 0; i < num; i++)
	{
		// フレーム名称を取得
		std::string frameName = MV1GetFrameName(trans_.modelId, i);
